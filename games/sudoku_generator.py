...
def sudokuGen():
    ...
    while len(cells) != 0:
        ...
        if choiceElement.solvedMethod(
        ) == False:  ##the actual setting of the cell
    ...
    for i in range(len(sudoku)):
        for n in range(len(sudoku)):
            if i != n:
                ...
                if position1[0] == position2[0] or position1[1] == position2[
                        1] or position1[2] == position2[2]:
    ...
    while solvedCells != []:
        for n in solvedCells:
            ...
            for i in cells:  ##now we itterate through the remaing unset cells and remove the input if it's in the same row, col, or box
                ...
                if position1[2] == position2[2]:
                    copy_s[i].remove(finalValue)
                if copy_s[i].lenOfPossible(
                ) == 1 and i not in solvedCells and i in cells:
                    solvedCells.append(i)
                ##print(n)
