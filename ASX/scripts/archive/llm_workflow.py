...
try:
    ...
    from db_models import CatalystItem, CatalystMaster
    from db_schemas import CatalystSchema
    from utils import get_root_dir, logger
except ImportError:
    from scripts.db_manager import db
    from scripts.db_models import CatalystItem, CatalystMaster
    from scripts.db_schemas import CatalystSchema
    from scripts.utils import get_root_dir, logger
                ...
                export_data.append(
                    {
                        ...
                        "Timeline": [
                            {"Time": i.label, "Event": i.content} for i in active_items if i.item_type == "milestone"
                        ],
                    }
                )
            ...
            with db.session_scope() as sess:
                ...
                for item_data in data:
                    try:
                        ...
                        db.sync_list_data(
                            sess, CatalystItem, "symbol", master.symbol, valid_c.Catalysts, item_type="catalyst"
                        )
                        db.sync_list_data(sess, CatalystItem, "symbol", master.symbol, valid_c.Risks, item_type="risk")
                        db.sync_list_data(
                            sess, CatalystItem, "symbol", master.symbol, valid_c.Timeline, item_type="milestone"
                        )
